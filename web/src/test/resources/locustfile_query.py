...
urls2 = ["/api/genes/query?query={\"name\":\"BRCA2\"}&facets=genome",
...
urls3 = ["/api/genes/query?query={\"name\":\"BRCA2\",\"genome\":\"homo_sapiens\"}&fields=[\"name\",\"description\",{\"homologues\":[\"division\",\"name\"]}]",
...
class UserBehavior(TaskSet):
    ...
    @task(1)
    def profile(self):
        """ lower weight for uncommonly used URLs """
        self.client.get(random.choice(urls3))

    @task(2)
    def profile(self):
        self.client.get(random.choice(urls2))

    @task(3)
    def profile(self):
        """ higher weight for commonly used URLs """
