...
public class VariantSearch extends JoinMergeSearch {
	...
	public VariantSearch(SearchRegistry provider) {
		...
		if (geneSearch != null) {
			joinTargets.put(SearchType.GENES, JoinStrategy.as(MergeStrategy.APPEND, "genes", "annot.ct.ensg"));
		}
		Search transcriptSearch = provider.getSearch(SearchType.TRANSCRIPTS);
		if (transcriptSearch != null) {
			joinTargets.put(SearchType.TRANSCRIPTS, JoinStrategy.as(MergeStrategy.APPEND, "transcripts", "annot.ct.enst"));
		}
	}
